...
- 例如 `yearLimit=30` 且 `maxYearsPerRequest=10` 时，会拆成：
  ...
  - `11-20`
  - `21-30`
- 当前默认仍然由第一段结果提供总评、事业、财富、婚姻、健康、家庭等分析字段，后续分段主要补 `chartPoints`。
- Debug 模式开启后，页面会显示：
  - 提交给后端的请求体
  - 后端原始响应
  - 模型原始文本
...
- 分段请求虽然降低了单次输出长度，但仍可能出现：
  ...
  - 第一段总评与后续分段风格不完全一致
- 当前实现还没有“总纲推演 + 分段推演”的双阶段结构，仍然是“第一段兼任总评 + 分段输出”。
- 这意味着：
  - 一致性比单独先做总纲要弱
  - 但实现复杂度更低
...
- 如果后续要提升准确性和一致性，优先考虑“总纲推演 + 分段流年推演”的两阶段方案，而不是继续把更多内容塞进单个 Prompt。
